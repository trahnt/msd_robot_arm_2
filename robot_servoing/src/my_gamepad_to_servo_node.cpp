                     ...
                     std::unique_ptr<control_msgs::msg::JointJog>& joint)
{
  ...
  if (dpadPressed || abxyPressed || lrPressed)
  {
    ...
    joint->joint_names.push_back("Revolute2");
    joint->velocities.push_back(-axes[D_PAD_Y]);

    joint->joint_names.push_back("Revolute3");
...
namespace robot_servoing
{
class MyJoyToServoPub : public rclcpp::Node
{
public:
  MyJoyToServoPub(const rclcpp::NodeOptions& options)
    : Node("my_joy_to_twist_publisher", options), frame_to_publish_(BASE_FRAME_ID)
  {
    // Setup pub/sub
...
RCLCPP_COMPONENTS_REGISTER_NODE(robot_servoing::MyJoyToServoPub)
